...
function fixture() {
  ...
  db.prepare('UPDATE remote_sync SET session_id=?,device_id=?,stream_epoch=?,sync_environment=?,source_seq=90,ack_seq=84,needs_snapshot=0 WHERE local_id=?')
    .run('session', 'device', 'epoch', environment, 'local');
  ...
  const claim: DeletionClaim = { operation: { operationId: 'op', sessionId: 'session', deviceId: 'device', deletionVersion: '1', stateVersion: '2', state: 'processing', action: RemoteDeletion.StopAndDelete, approvedGuard: guard },
    target: { ...owner, serviceScope: environment, deviceId: 'device', sessionId: 'session', localSessionId: 'local', streamEpoch: 'epoch' },
  ...
  const completion = (receipt: DeletionReceipt) => ({ operationId: 'op', deletionVersion: '1', proofKind: 'remote_execution', owner, serviceScope: environment,
  ...
  const deps = { store, service, runtime, context: () => ({ owner, environment, deviceId: 'device', generation: '1', enabled }), request, reconcileStop: vi.fn(async () => false) };
